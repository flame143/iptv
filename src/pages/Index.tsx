    ...
    <div className="h-screen w-full bg-black text-white p-2 md:p-4 lg:p-6 overflow-hidden">
      ...
      <div className="h-full w-full flex flex-col bg-[#0a0a0a] tunex-glow rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border border-white/5 relative">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          {/* PLAYER ZONE (70%) */}
          <div className="flex-1 p-4 md:p-8 flex flex-col items-center justify-center bg-[#050505] overflow-y-auto">
            {selectedChannel ? (
              <div className="w-full max-w-5xl flex flex-col gap-6 animate-in fade-in duration-500">
                ...
                <div className="w-full bg-[#0d0e12] border border-white/5 rounded-2xl p-5 md:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xl">
                  <div className="flex items-center gap-4">
                    ...
                    {selectedChannel.logo && !logoError[selectedChannel.id] ? (
                      <img 
                        ...
                        className="w-12 h-12 rounded-xl object-contain bg-zinc-950 p-1 border border-white/10 shrink-0" 
                      />
                    ) : (
                      <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-[#00FF00]/10 text-[#00FF00] font-black text-sm flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(0,255,0,0.05)]">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                        <h2 className="text-lg md:text-xl font-black text-white uppercase tracking-wide leading-none">{selectedChannel.name}</h2>
                      ...
                      <p className="text-xs text-zinc-500 font-semibold tracking-wide flex items-center gap-2 flex-wrap">
                        <span>Format: <span className="text-zinc-300 font-bold uppercase">{selectedChannel.type}</span></span>
                        <span className="text-zinc-700">•</span>
                        <span>Proxy: <span className={selectedChannel.useProxy ? "text-[#00FF00]" : "text-zinc-500"}>{selectedChannel.useProxy ? "Active Proxy" : "Direct Connection"}</span></span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <Button 
                      ...
                      variant="outline" 
                      className={cn(
                        "rounded-xl h-11 px-5 text-xs font-bold uppercase tracking-wider gap-2 border-white/10 bg-zinc-900/50 hover:bg-zinc-800 transition-all",
                        copiedId === selectedChannel.id ? "text-[#00FF00] border-[#00FF00]/30" : "text-zinc-300 hover:text-white"
                      )}
                    >
                      {copiedId === selectedChannel.id ? (
                        <>
                          <Check className="w-4 h-4" /> Copied Stream!
                        </>
                      ) : (
                        <>
